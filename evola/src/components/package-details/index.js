        ...
        <div>
            ...
            <div className="flex">
                ...
                <div className='px-6 py-2'>
                    <input class="form-check-input appearance-none border border-gray-300 rounded-sm bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" type="checkbox" value={overridePriceCheck} onChange={handleOverridePriceCheck} disabled={isDisabled()} id="overridePrice" />
                    <label class="form-check-label inline-block text-gray-800" for="overridePrice">
                        Override Price
                    </label>
                </div>
            </div>
            {overridePriceCheck ? <div class="flex-wrap bg-green-100 border-l-4 my-2 border-green-300 rounded-md w-full px-6 py-4 cursor-pointer">
                <div className='flex justify-between'>
                    <label className="mr-2" for="input-Isk">
                        Total Sell Price
                    </label>
                    <input id="input-Isk" type='number' step="0.1" min='0' placeholder="0" value={overridePrice} onChange={handleOverridePrice} />
                </div>
                <div className='flex justify-between'>
                    <label className="mr-2" for="input-volume">
                        Total Volume
                    </label>
                    <input id="input-volume" type='number' step="0.1" min='0' placeholder="0" value={overrideVolume} onChange={handleOverrideVolume}/>
                </div>
            </div> : ""}
            {errorMessage ? <div class="bg-red-100 border-l-4 border-red-300 my-2 rounded-md w-full px-6 py-4 cursor-pointer">{errorMessage}</div> : ""}
