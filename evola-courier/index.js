...
const unique = Array.from(
  new Map(data.map(item => [item.character_id, item])).values()
);

const API_URL = 'http://localhost:3000/member'; // Adjust if needed

async function postMember(character_id) {
  try {
    ...
    if (!res.ok) {
      console.warn(`[WARN] Failed for ${character_id} (${res.status})`);
    } else {
      console.log(`[OK] Queued ${character_id}`);
    }
  } catch (err) {
    console.error(`[ERROR] ${character_id}`, err.message);
  }
}

async function processAll() {
  for (const entry of unique) {
    if (!entry.character_id) continue;
    await postMember(entry.character_id);
  }
}
